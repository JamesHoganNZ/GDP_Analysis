...
##
   ...
   ##
      load("Data_Output/ConstantPrice_SA_Qtr_GDP_Published20251114.rda")
      load("Data_Output/ConstantPrice_Actual_Annual_CapitalStock_Published20251114.rda")
      load("Data_Output/ConstantPrice_Actual_Qtr_PaidHours_Published20251114.rda")
      load("Data_Output/Household_Labour_Force_Survey_Published20251114.rda")
      load("Data_Output/CurrentPrice_Actual_Qtr_CPIAllGroup_Published20251114.rda")
   ...
   ##
      GDP        <- unique(ConstantPrice_SA_Qtr_GDP_Published20251114$Industry)
      CapStock   <- unique(ConstantPrice_Actual_Annual_CapitalStock_Published20251114$Industry)
      Labour     <- unique(ConstantPrice_Actual_Qtr_PaidHours_Published20251114$Industry)
      Unemployed <- Household_Labour_Force_Survey_Published20251114
      CPI        <- data.frame(CurrentPrice_Actual_Qtr_CPIAllGroup_Published20251114)
      ...
      GDP <- merge(ConstantPrice_SA_Qtr_GDP_Published20251114,
                   Reclassify_Industry,
                   by.x = c("Industry"),
                   by.y = c("CapStock"))

      CapStock <- merge(ConstantPrice_Actual_Annual_CapitalStock_Published20251114,
                        Reclassify_Industry,
                        by.x = c("Industry"),
                        by.y = c("CapStock"))
                        
      Labour <- ConstantPrice_Actual_Qtr_PaidHours_Published20251114
   ...
   ##
     ...
     ##  
        ...
        ##
            ...
            ggsave("Graphical_Output/New Zealand Production - Update2025.png", height =(1.5)*16.13, width = (1.75)*20.66, dpi = 165, units = c("cm"))
            ...
            ggsave("Graphical_Output/New Zealand Production - COVID and Since - Update2025.png", height =(1.5)*16.13, width = (1.75)*20.66, dpi = 165, units = c("cm"))
        ##
...
#                  scale_colour_manual(aesthetics = c("colour", "fill"), values = nzier.cols(), name="") +
      ...
      showtext_auto()
         ...
         ggplot(Output_Gap[(month(Output_Gap$TimePeriod) == 3),], aes(x=Output_Gap/100, y=Unemployment_Rate/100))     +
               ...
               ##
                geom_point(size = 3, colour = "red", data = Output_Gap[(month(Output_Gap$TimePeriod) == 3) & (year(Output_Gap$TimePeriod) > 2024),]) +
                geom_text(aes(label=format(TimePeriod, "%Y")), size=7, nudge_x = 0.003, colour = "red", data = Output_Gap[(month(Output_Gap$TimePeriod) == 3) & (year(Output_Gap$TimePeriod) > 2024),]) +
                geom_path(size = 1, linejoin = "mitre", lineend = "butt", colour = "red", data = Output_Gap[(month(Output_Gap$TimePeriod) == 3) & (year(Output_Gap$TimePeriod) > 2023),]) +
         ...
         ggsave("Graphical_Output/Okuns Law - System_of_Equations - Update2025.png", height =(1.5)*16.13, width = (1.75)*20.66, dpi = 165, units = c("cm"))
